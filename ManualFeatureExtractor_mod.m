vars2keep = {'Cnt', 'all_feats_unwarped'};
clearvars('-except',vars2keep{:});
action = "PUSH";
if(~exist('Cnt','var'))
    Cnt = 1; %Use default warping window
end

%Abductor pollicis brevis (APB), Flexor carpi radialis (FCR), Dorsal interossei (DI), and Extensor carpi radialis (ECR)
frcFolder = 'E:\Rabbit\2020.06.03 (Partial Experiment)\xiongjing-02\force data\';
emgFolder = 'E:\Rabbit\2020.06.03 (Partial Experiment)\xiongjing-02\emg data\';
emFolder =  'E:\Rabbit\2020.06.03 (Partial Experiment)\xiongjing-02\EM data\';
glvFolder = 'E:\Rabbit\2020.06.03 (Partial Experiment)\xiongjing-02\glove data\';
...
%An approach with fixed window length and variable window number is adopted
    ...
    for i = 1:size(emg.Data,2)
        ...
        for j = 1:rEMG
            ...
            featEMGARV(j,i) = mean(abs(emg.Data(window, i)));  
        end
    end
...
%save data
if action == "PULL"
    klass = 1*ones(length(featEMGRMS),1);
elseif action == "PUSH"
    klass = 2*ones(length(featEMGRMS),1);
end

all_feats_unwarped(Cnt : Cnt+length(featEMGRMS)-1, :) = [featEMGRMS, featEMGARV,...
 ...
 featGLintraTNF, featGLintraINF, featGLintraMNF, featGLintraRNF, featGLintraLNF, featClampForce, klass];


Cnt = Cnt+length(featEMGRMS);