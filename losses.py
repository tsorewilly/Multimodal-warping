...
def figPlot(y_true, y_pred, ax_fz=10, fz=11, saveAs='foo.png', title=''):
    ...
    if (len(y_true) > 6):
        figsize = [9, 9];
    plot_confusion_matrix_from_data(y_true, y_pred, columns=categories, 
                                    annot=True, cmap='Oranges', 
                                    fmt='.2f', ax_fz=ax_fz, fz=fz,
                                    lw=0.5, cbar=False, figsize=[6, 6], 
