...
prefix = 'E:\Animal Experiment -01\';
paths = {'2020.05.22 (Complete Experiment)','2020.05.28','2020.06.01 (Complete Experiment)',...
        '2020.06.02 (Complete Experiment)', '2020.06.03 (Partial Experiment)'};

for p=1:size(paths,2)
    pathDir = 'G:\Documents\My Postdoc Work\Intelligent Robot Navigation\EMG\Data\'; 
    Lists=dir([prefix, paths{p}]);
    recordLists = {Lists.name};   
    
    for sj=3 : length(recordLists)
        %Locate each timeseries data folders
        frcFolder = [[prefix, paths{p}], '\', recordLists{sj}, '\force data\'];     frcFiles = arrangeFiles(frcFolder);
        emgFolder = [[prefix, paths{p}], '\', recordLists{sj}, '\emg data\'];       emgFiles = arrangeFiles(emgFolder);
        emFolder =  [[prefix, paths{p}], '\', recordLists{sj}, '\EM data\'];        emFiles = arrangeFiles(emFolder);
        glvFolder = [[prefix, paths{p}], '\', recordLists{sj}, '\glove data\'];     glvFiles = arrangeFiles(glvFolder);
        ...
        figure(1); clf;
            ...
            subplot(427);plot(emg.Data(emg.AlignStart : emg.AlignEnd,8)); xlabel('time (msec)');ylabel('EMG value (mV)');title('Extensor carpi radialis EMG value'); 
        ...
        %An approach with fixed window length and variable window number is adopted
            EMGWinLent = 2000;  %Every 2 seconds
        ...
        %if(fileEMG==0 || fileForce==0)
            figure(3);clf
            subplot(411); plot(featEMGRMS(:,1),'-r*'); hold on; plot(featEMGRMS(:,2),'-b*');  plot(featEMGRMS(:,3),'-k*');  plot(featEMGRMS(:,4),'-g*'); hold off;
            ylabel('Root mean square');     xlabel('Window Index'); title('RMS Analysis Window');   legend('APB','FCR','DI','ECR', 'Location','best', 'NumColumns', 2);
            subplot(412); plot(featEMGSamEn(:,1),'-r*'); hold on; plot(featEMGSamEn(:,2),'-b*');  plot(featEMGSamEn(:,3),'-k*');  plot(featEMGSamEn(:,4),'-g*'); hold off;
        ...
        %     ylabel('Sample Entropy');     xlabel('Window Index');   title('Sample Entroy Analysis Window');  legend('APB','FCR','DI','ECR', 'Location','best', 'NumColumns', 2);
            subplot(413); plot(featEMGMPF(:,1),'-r*'); hold on; plot(featEMGMPF(:,2),'-b*');  plot(featEMGMPF(:,3),'-k*');  plot(featEMGMPF(:,4),'-g*'); hold off;
            ...
            subplot(414); plot(featEMGEntroy(:,1),'-r*'); hold on; plot(featEMGEntroy(:,2),'-b*');  plot(featEMGEntroy(:,3),'-k*');  plot(featEMGEntroy(:,4),'-g*'); hold off;
            ylabel('Feature Entroy');     xlabel('Bin Index');      title('Entroy Analysis Window');    legend('APB','FCR','DI','ECR', 'Location','best', 'NumColumns', 2);

            figure(4);clf
            subplot(2,2,[1 2]); plot(force.Data, '-r'); ylabel('Proximal Force (gram)');     xlabel('Time'); title('Analysis of Catheterization Force');
            subplot(223); plot(featClampForce,'-r*');
            subplot(224); histfit(force.Data)

            figure(5); clf    
            plot(featEMinterHand3D, '-ro'); hold on;     plot(featEMintraIndex3D,'-k*');    plot(featEMintraThumb3D,'-bp');
        %    subplot(224); histfit(fVals)
            ...
            figure(6);clf
            subplot(311); plot(featGLinterTIS,'-r*'); hold on; plot(featGLinterIMS,'-b*');  plot(featGLinterMRS,'-k*');  plot(featGLinterRLS,'-g*'); hold off;
            ylabel('RMS Values');     xlabel('Window Index'); title('Inter-Finger Sensor Data');   legend('TIS','TMS','TRS','RLS', 'Location','best', 'NumColumns', 2);
            subplot(312); plot(featGLinterTIF,'-r*'); hold on; plot(featGLinterIMF,'-b*');  plot(featGLinterMRF,'-k*');  plot(featGLinterRLF,'-g*'); hold off;
            ...
            subplot(313); plot(featGLintraTNF,'-r*'); hold on; plot(featGLintraINF,'-b*');  plot(featGLintraMNF,'-k*');  plot(featGLintraRNF,'-g*'); plot(featGLintraLNF,'-c*'); hold off;
